...
contract('HierarchyDaoFactory', (accounts) => {
	...
	before(async () => {
		...
		// const hierarcyDaoAutoAddress = await hierarchyDaoFactory.hierarcyDaoAuto();
		// hierarcyDaoAuto = DaoBaseAuto.at(hierarcyDaoAutoAddress);
	...
	/*it("boss should be able to manage groups only by voting", async () => {
		await hierarcyDaoAuto.addGroupMemberAuto("ANY_GROUP", employee1, { from: boss }).should.be.fulfilled;
	});

	it("manager should be able to manage groups only by voting", async () => {
		await hierarcyDaoAuto.addGroupMemberAuto("ANY_OTHER_GROUP", employee1, { from: manager1 }).should.be.fulfilled;
	});
