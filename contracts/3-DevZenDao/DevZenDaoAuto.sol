...
contract DevZenDaoAuto is GenericCaller {
	...
	// function updateDaoParamsAuto(Params _params) public returns(address proposalOut) {
	...
	function withdrawEtherAuto(address _output) public returns(address proposalOut) {
		...
		return doAction(WITHDRAW_ETHER, dao, msg.sender, "withdrawEtherGeneric(bytes32[])", params);
	}

	function selectNextHostAuto(address _nextHost) public returns(address proposalOut) {
		bytes32[] memory params = new bytes32[](1);
		params[0] = bytes32(_nextHost);
		return doAction(SELECT_NEXT_HOST, dao, msg.sender, "selectNextHostGeneric(bytes32[])", params);
	}

	function changeTheGuestAuto(address _guest) public returns(address proposalOut) {
		bytes32[] memory params = new bytes32[](1);
		params[0] = bytes32(_guest);
		return doAction(CHANGE_GUEST, dao, msg.sender, "changeTheGuestGeneric(bytes32[])", params);
	}

	function emergency_ChangeTheGuestAuto(address _guest) public returns(address proposalOut) {
		bytes32[] memory params = new bytes32[](1);
		params[0] = bytes32(_guest);
		return doAction(EMERGENCY_CHANGE_GUEST, dao, msg.sender, "emergency_ChangeTheGuestGeneric(bytes32[])", params);
	}
	...
	function moveToNextEpisodeAuto(bool _guestHasCome) public returns(address proposalOut) {
		...
		return doAction(MOVE_TO_NEXT_EXPISODE, dao, msg.sender, "moveToNextExpisodeGeneric(bytes32[])", params);
	}
